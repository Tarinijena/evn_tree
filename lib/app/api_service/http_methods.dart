...
class HttpMethodsDio {
  ...
  getMethodWithToken({required String api, required Function fun, required String? token}) async {
    if (kDebugMode) {
      print("<<jks>>>>>API_CALL>>>>>>\n\n\n\n\n$api");
    }
    try {
  ...
  postMethodWithToken1({required String api,required dynamic json, required Function fun, required String? token}) async {
    try {
      if (kDebugMode) {
        print("API NAME:>$api");
      }
